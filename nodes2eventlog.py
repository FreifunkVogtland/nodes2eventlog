...
def extract_eventfeed(eventlog):
    ...
    for event in feedlog:
        ...
        entry.title = "[%s] %s" % (event['eventtype'].upper(),
                                   event['message'])
        entry.content = "[%s] %s" % (event['eventtype'].upper(),
                                     event['message'])
...
def sweep_nodes(state, eventlog):
    ...
    for node_id, node in removed.items():
        log_event_node(eventlog, datetime.datetime.utcnow(), "drop", node_id,
                       node)
...
def sanitize_nodestate(nodestate):
    if 'online' not in nodestate:
        nodestate['online'] = False

    if 'hostname' not in nodestate:
        nodestate['hostname'] = ""
...
def parse_nodestate(nodes, eventlog, state, graveyard):
    ...
    for node in nodes['nodes']:
        ...
        adjust_graveyard(graveyard, node_id, state[node_id], firsttimestamp,
                         timestamp)

        if new_node:
            log_event_node(eventlog, firsttimestamp, "new", node_id,
                           state[node_id])
        ...
        if oldstate_online != state[node_id]['online'] or new_node:
            ...
            log_event_node(eventlog, timestamp, eventtype, node_id,
                           state[node_id])

        if oldstate_hostname != "" and oldstate_hostname != state[node_id]['hostname']:
            url = MAP_NODE_URL + node_id
            log_event(eventlog, timestamp,
                      "RENAME", "%s -> %s" % (oldstate_hostname,
                                              state[node_id]['hostname']), url)
